...
{
    int         ret                  = MSP_SUCCESS;
    const char* login_params         = "appid = 5e1d6a08, work_dir = .";//登录参数,appid与msc库绑定,请勿随意改动
    const char* session_begin_params = "engine_type =local, text_encoding = UTF8, tts_res_path = fo|/Robot/voice/bin/msc/res/tts/xiaoyan.jet;fo|/Robot/voice/bin/msc/res/tts/common.jet, sample_rate = 16000, speed = 50, volume = 50, pitch = 50, rdn = 0";
    /* 用户登录 */
...
int main(int argc,char **argv)
{
    ...
    ros::Subscriber sub =n.subscribe("voiceWords",10,xfcallback);
    ros::spin();//循环
    return 0;
}
